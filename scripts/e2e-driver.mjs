...
const exprs = {
  ...
  add: `window.dshc.addPlugin(${JSON.stringify(arg)}).then(v => ({ ok: true, v }), e => ({ ok: false, e: String(e) }))`,
  plugins: `window.dshc.listPlugins()`,
  enable: `window.dshc.enablePlugin(${JSON.stringify(arg)})`,
  disable: `window.dshc.disablePlugin(${JSON.stringify(arg)})`,
  ...
  enableTest: `window.dshc.enablePlugin('dsh-desktop-test-plugin')`,
  remove: `window.dshc.removePlugin(${JSON.stringify(arg)})`,
  uninstall: `window.dshc.uninstallPlugin(${JSON.stringify(arg)}).then(v => ({ ok: true, v }), e => ({ ok: false, e: String(e) }))`,
  export: `window.dshc.exportPlugin(${JSON.stringify(arg)})`,
